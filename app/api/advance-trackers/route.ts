...
// Generate parent upsell email (separate from kid's Jingles email)
function generateParentUpsellEmail(
  parentFirstName: string,
  childName: string,
  dayNumber: number
): { subject: string; html: string } {
  
  const subject = dayNumber === 1 
    ? `🎅 A quick note about ${childName}'s Santa letter...`
    : `✨ Last chance to make ${childName}'s Christmas extra magical!`

  const urgency = dayNumber === 1
    ? `${childName}'s letter is on its magical journey to Santa right now!`
    : `${childName}'s letter arrives at Santa's desk tomorrow!`
    ...
    <tr>
      <td align="center" style="padding: 40px 20px;">
        <table role="presentation" width="600" cellspacing="0" cellpadding="0" style="max-width: 600px; width: 100%;">
          <tr>
            <td>
              <table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="background-color: #ffffff; border-radius: 20px; box-shadow: 0 4px 6px rgba(0,0,0,0.1);">
                <tr>
                  <td style="background: linear-gradient(135deg, #c41e3a 0%, #8b0000 100%); height: 8px; border-radius: 20px 20px 0 0;"></td>
                </tr>
                <tr>
                  <td style="padding: 40px;">
                    ...
                    <p style="color: #333; font-size: 16px; line-height: 1.6; margin: 0 0 20px 0;">
                      I wanted to let you know about something special you can add to make this experience even more magical...
                    </p>
                    
                    <table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="background-color: #fef9e7; border-radius: 15px; border: 2px solid #d4af37; margin: 25px 0;">
                      <tr>
                        <td style="padding: 25px;">
                          <p style="color: #8b6914; font-size: 18px; font-weight: bold; margin: 0 0 15px 0; text-align: center;">
                            ✨ Santa's Magic Upgrade - $7.99 ✨
                          </p>
                          <p style="color: #333; font-size: 15px; line-height: 1.8; margin: 0;">
                            <strong>🎅 Personal Letter FROM Santa</strong><br>
                            Santa writes back to ${childName} by name, mentioning their wishes and the good things they've done this year.<br><br>
                            <strong>⭐ Official Nice List Certificate</strong><br>
                            A beautiful, printable certificate confirming ${childName} is on Santa's Nice List.<br><br>
                            <strong>🎬 Magical Video Journey</strong><br>
                            Watch Jingles the Elf carry ${childName}'s letter through 5 stunning North Pole locations!
                          </p>
                        </td>
                      </tr>
                    </table>
                    ...
                    <table role="presentation" cellspacing="0" cellpadding="0" style="margin: 30px auto;">
                      <tr>
                        <td style="background: linear-gradient(135deg, #c41e3a 0%, #8b0000 100%); border-radius: 30px;">
                          <a href="https://officiallettertosanta.com/tracker-login" style="display: inline-block; color: #ffffff; font-size: 18px; font-weight: bold; padding: 18px 35px; text-decoration: none;">
                            🎁 Upgrade ${childName}'s Experience
                          </a>
                        </td>
                      </tr>
                    </table>
                    ...
                    <p style="color: #666; font-size: 14px; line-height: 1.6; margin: 25px 0 0 0; text-align: center;">
                      Just log in with your last name and passcode to upgrade.
                    </p>
                    
                    <p style="color: #333; font-size: 16px; line-height: 1.6; margin: 30px 0 0 0;">
                      The look on their face when Santa writes back? That's what Christmas memories are made of. 🎄
                    </p>
                    
                    <p style="color: #333; font-size: 16px; line-height: 1.6; margin: 25px 0 0 0;">
                      Warmly,<br>
                      The Letters to Santa Team
                    </p>
                  </td>
                </tr>
                <tr>
                  <td style="background-color: #f9f9f9; padding: 20px; text-align: center; border-radius: 0 0 20px 20px;">
                    <p style="color: #999; font-size: 12px; margin: 0;">
                      This is a quick note for parents only - ${childName}'s magical Jingles story was sent separately! 🧝
                    </p>
                  </td>
                </tr>
              </table>
...
// Generate beautiful HTML email for Jingles story updates (for kids!)
...
This has been the most magical journey, and I'm so glad I got to be your letter's guide. Thank you for trusting me with something so special!`,
  ...
  if (dayNumber === 4 && isMagicTier && childId) {
    const baseUrl = process.env.NEXT_PUBLIC_BASE_URL || 'https://officiallettertosanta.com'
...
export async function POST(request: NextRequest) {
  ...
  try {
      ...
      if (newIndex >= 1 && newIndex <= 4) {
        ...
        if (webhookUrl) {
          try {
            ...
            // Send Jingles story email (for kids)
            ...
            // Send parent upsell email on day 1 and 2 for FREE tier only
            if (!isMagicTier && (newIndex === 1 || newIndex === 2)) {
              const upsell = generateParentUpsellEmail(
                child.customer.firstName,
                child.name,
                newIndex
              )
    ...
    return NextResponse.json({
      ...
      emailsSent,
      upsellsSent,
      message: `Advanced ${advancedCount} trackers, sent ${emailsSent} story emails and ${upsellsSent} upsell emails`
    })
